    ...
    <div class="neologisme-cards">
      <div v-for="(value, index) in neologismes" :key="index">
        ...
        <b-dropdown text="Acciones" v-if="login.linguist || login.admin">
          <b-dropdown-item v-on:click="submit(value.id)"
            >Aceptar propuesta</b-dropdown-item
          >
          <b-dropdown-item
            :to="{
              name: 'rp-neologismes',
            ...
            >Rechazar propuesta</b-dropdown-item
          >
          <b-dropdown-item
            v-if="login.admin"
            style="color: red; !important"
            v-on:click="deleteData(value.id)"
            >Eliminar propuesta</b-dropdown-item
          >
        </b-dropdown>
        <b-button
          class="bttn-app"
...
export default {
  created() {
    axios.get("http://localhost:3000/login/1").then((response_l) => {
      this.login = response_l.data;

      axios
        .get("http://localhost:3000/users/" + response_l.data.user_id)
        .then((response_u) => {
          this.form_user = response_u.data;
          axios.get("http://localhost:3000/neologismes").then((response) => {
            if (response_l.data.admin || response_l.data.linguist) {
              ...
              for (let index = 0; index < response.data.length; index++) {
                if (
                  response_u.data.proposals.includes(response.data[index].id)
                ) {
                  this.neologismes.push(response.data[index]);
                }
              }
  ...
  methods: {
    submit(id) {
      ...
      axios
        .get("http://localhost:3000/users/" + this.neologismes.user[0].user_id)
        .then((response_u) => {
          for (
            let index = 0;
          ...
          axios.patch(
            "http://localhost:3000/users/" + this.neologismes.user[0].user_id,
            payload3
          );
        });
