        ...
        <div class="neologisme-cards">
            <div v-for="(value,index) in neologismes" :key="index">
                ...
                <div>
                 ...
                 <b-button class="bttn-app" :to="{name: 'fv-neologismes',params: { userid: $route.params.userid ,neoId: value.id}}"> +</b-button>
            </div>
        </div>
          ...
          .then(response_log => {
            ...
            axios.get('http://localhost:3000/users/' + this.$route.params.userid)
              ...
              axios.get('http://localhost:3000/neologismes')
                .then(response_neo => {      
                    for (let index = 0; index < response_users.data.fav_neo.length; index++) {
                        this.neologismes.push(response_neo.data[response_users.data.fav_neo[index]-1]);
                    }  
              })
            })
        })
